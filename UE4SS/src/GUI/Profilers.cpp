    ...
    auto render() -> void
    {
        ...
        else
        {
            if (ImGui::Button("Debug: Show Map Info"))
            {
                TRY([] {
                    ...
                    Output::send(STR("ClassToObjectListMap: Num={}\n"), Tables.NumClassEntries());

                    auto* ActorClass = AActor::StaticClass();
                    size_t exact = 0;
                    Tables.ForEachObjectOfClass(ActorClass, [&](UObjectBase*) { ++exact; });
                    size_t derived = 0;
                    Tables.ForEachObjectOfClassIncludingDerived(ActorClass, [&](UObjectBase*) { ++derived; });
                    Output::send(STR("AActor::StaticClass() = {}\n"), static_cast<void*>(ActorClass));
                    Output::send(STR("Exact AActor instances: {}, including derived: {}\n"), exact, derived);
                });
            }
        }
        ...
        if (ImGui::Button("Test: HashTable AActor"))
        {
            TRY([] {
                ...
                size_t count = 0;
                FUObjectHashTables::Get().ForEachObjectOfClass(AActor::StaticClass(), [&](UObjectBase*) {
        ...
        if (ImGui::Button("Test: HashTable UClass"))
        {
            TRY([] {
                HASHTABLE_PROFILE_SEARCH_BEGIN()
                size_t count = 0;
                FUObjectHashTables::Get().ForEachObjectOfClass(UClass::StaticClass(), [&](UObjectBase*) {
                    HASHTABLE_PROFILE_OBJ_COUNT()
                    ++count;
                });
                if (count > 0)
