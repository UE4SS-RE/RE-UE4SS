...
namespace RC::UEGenerator
{
    ...
    auto UEHeaderGenerator::generate_module_build_file(const SystemStringType& module_name) -> void
    {
        ...
        module_build_file.append_line(std::format(SYSSTR("public class {} : ModuleRules {{"), module_name_sys));
        module_build_file.begin_indent_level();

        module_build_file.append_line(std::format(SYSSTR("public {}(ReadOnlyTargetRules Target) : base(Target) {{"), module_name_sys));
        module_build_file.begin_indent_level();
        ...
        {
            module_build_file.append_line(std::format(SYSSTR("\"{}\","), to_system(other_module_name)));
        }
    ...
    auto UEHeaderGenerator::generate_module_implementation_file(const SystemStringType& module_name) -> void
    {
        ...
        {
            module_impl_file.append_line(std::format(SYSSTR("IMPLEMENT_MODULE(FDefaultGameModuleImpl, {});"), module_name_sys));
        }
        else
        {
            module_impl_file.append_line(std::format(SYSSTR("IMPLEMENT_PRIMARY_GAME_MODULE(FDefaultGameModuleImpl, {}, {});"), module_name_sys, module_name_sys));
        }
    ...
    auto UEHeaderGenerator::generate_struct_definition(UScriptStruct* script_struct, GeneratedSourceFile& header_data) -> void
    {
        ...
        header_data.append_line(
                std::format(SYSSTR("struct {}{}{} {{"), is_struct_exported ? api_macro_name : SYSSTR(""), struct_native_name, parent_struct_declaration));
        header_data.begin_indent_level();
    ...
    auto UEHeaderGenerator::generate_enum_definition(UEnum* uenum, GeneratedSourceFile& header_data) -> void
    {
        ...
        auto enum_prefix = uenum->GenerateEnumPrefix();
        ...
        {
            auto enum_name = Name.ToString();
            auto result_enumeration_line = sanitize_enumeration_name(to_system_string(enum_name));
            ...
            {
                result_enumeration_line.append(std::format(SYSSTR(" = {}"), sanitize_enumeration_name(to_system_string(first_name_with_value))));
            }
    ...
    auto UEHeaderGenerator::generate_delegate_type_declaration(UFunction* signature_function, UClass* delegate_class, GeneratedSourceFile& header_data) -> void
    {
        ...
        SystemStringType delegate_declaration_string =
                std::format(SYSSTR("{}DECLARE_DYNAMIC{}{}_DELEGATE{}{}{}({}{}{}{});"),
        ...
        for (auto attachment : implementation_file.attachments)
        {
            ...
            else
            {
                generate_advanced_assignment_expression(attachment.first,
                                                        (attachment.second).attach_string,
                                                        implementation_file,
                                                        SYSSTR("this->"),
                                                        (attachment.second).property_type,
                                                        SYSSTR("->"));
            }
        }
                                              ...
                                              bool generate_as_override) -> void
    {
        ...
        const auto context_name = uclass->GetName();
        ...
        if (is_function_pure_virtual)
        {
            ...
            if (return_property != NULL)
            {
                const auto default_property_value = generate_default_property_value(return_property, header_data, to_system_string(context_name));
        ...
        auto function_argument_list =
                generate_function_parameter_list(uclass, function, header_data, false, to_system_string(context_name), blacklisted_property_names);
    ...
    auto UEHeaderGenerator::generate_enum_value(UEnum* uenum, int64_t enum_value) -> SystemStringType
    {
        ...
        for (auto [Name, Value] : uenum->ForEachName())
        {
            if (Value == enum_value)
            {
                enum_constant_name = sanitize_enumeration_name(to_system_string(Name.ToString()));
            }
        }
                                                                    ...
                                                                    const SystemStringType& operator_type) -> void
    {
        ...
        {
            implementation_file.append_line(
                    std::format(SYSSTR("(*p_{}->ContainerPtrToValuePtr<{}>(this)){}{};"), field_class_name, property_type, operator_type, value));
        }
        else
        {
            for (int32_t i = 0; i < property->GetArrayDim(); i++)
            {
                implementation_file.append_line(std::format(SYSSTR("*p_{}->ContainerPtrToValuePtr<{}>(this){}[{}]{}{};"),
                                                            field_class_name,
                                                            property_scope,
                                                            field_class_name,
                                                            i,
            ...
            UStruct* ustruct, FProperty* property, void* object, GeneratedSourceFile& implementation_file, const SystemStringType& property_scope) -> void
    {
        ...
        const auto property_type = generate_property_cxx_name(property, true, ustruct);
        ...
        if (property->IsA<FNameProperty>())
        {
            ...
            {
                const auto result_property_value = std::format(SYSSTR("TEXT(\"{}\")"), to_system(name_value_string));
                if (!super_and_no_access)
                {
                    generate_simple_assignment_expression(property, result_property_value, implementation_file, property_scope);
                }
        ...
        if (property->IsA<FStrProperty>())
        {
            ...
            const auto string_value_string = (to_system(string_value->GetCharArray()));

            // Ensure property either does not exist in parent class or is overriden in the CDO for the child class
            if (super_property != nullptr)
            {
                FString* super_string_value = super_property->ContainerPtrToValuePtr<FString>(super_object);
                const auto super_string_value_string = (to_system(super_string_value->GetCharArray()));
        ...
        if (property->IsA<FObjectProperty>())
        {
            ...
            if (sub_object_value->HasAnyFlags(EObjectFlags::RF_DefaultSubObject))
            {
                ...
                if (auto it = m_class_subobjects.find(object_name); it != m_class_subobjects.end())
                {
                    ...
                    initializer = to_system(it->second);
                    FProperty* prior_property = ustruct->GetPropertyByNameInChain(it->second.c_str());
                ...
                else if ((super_object_class_type && object_class_type != super_object_class_type) || parent_component_found)
                {
                    ...
                    implementation_file.m_implementation_constructor.append(
                            std::format(SYSSTR(".SetDefaultSubobjectClass<{}>(TEXT(\"{}\"))"), get_native_class_name(object_class_type), to_system(object_name)));
                ...
                else
                {
                    ...
                    initializer = std::format(SYSSTR("CreateDefaultSubobject<{}>(TEXT(\"{}\"))"), object_class_name, to_system(object_name));
                    m_class_subobjects.try_emplace(object_name, (property->GetName()));
                    if (!super_and_no_access)
                    {
                        generate_simple_assignment_expression(property, initializer, implementation_file, property_scope);
                ...
                if (attach_parent_object_value != NULL)
                {
                    ...
                    {
                        // Set property to equal previous property referencing the same object
                        attach_string = std::format(SYSSTR("SetupAttachment({})"), to_system(it->second));
                        parent_found = true;
                    }
                    else if (as_class)
                    {
                            ...
                            if (check_property->IsA<FObjectProperty>())
                            {
                                ...
                                if (check_sub_object_value)
                                {
                                    ...
                                    if (check_object_name == attach_parent_object_name)
                                    {
                                        ...
                                        {
                                            auto parent_property_name =
                                                    std::format(SYSSTR("const FProperty* p_{}_Parent = GetClass()->FindPropertyByName(\"{}\");"),
        ...
        if (property->IsA<FArrayProperty>())
        {
            ...
            if (property_value->Num() > 0)
            {
                ...
                {
                    implementation_file.append_line(
                            std::format(SYSSTR("{}{}.AddDefaulted({});"), property_scope, to_system(property->GetName()), (int32_t)property_value->Num()));
                }
                else
                {
                    // TODO
    ...
    auto UEHeaderGenerator::generate_class_flags(UClass* uclass) const -> SystemStringType
    {
        ...
        const auto class_config_name = uclass->GetClassConfigName().ToString();
    ...
    auto UEHeaderGenerator::generate_property_type_declaration(FProperty* property, const PropertyTypeDeclarationContext& context) -> SystemStringType
    {
        ...
        throw std::runtime_error(
                RC::fmt("[generate_property_type_declaration] Unsupported property class '{}', full name: '{}'", field_class_name, property->GetFullName()));
    }
    //*/
    ...
    auto UEHeaderGenerator::generate_enum_flags(UEnum* uenum) const -> SystemStringType
    {
        ...
        if (UE4SSProgram::settings_manager.UHTHeaderGenerator.MakeEnumClassesBlueprintType)
        {
            ...
            if (cpp_form == UEnum::ECppForm::EnumClass)
            {
                ...
                if ((underlying_type == m_underlying_enum_types.end() && (get_highest_enum(uenum) <= 255 && get_lowest_enum(uenum) >= 0)) ||
                    ((underlying_type != m_underlying_enum_types.end()) && (underlying_type->second == SYSSTR("uint8"))))
                {
    ...
    auto UEHeaderGenerator::get_highest_enum(UEnum* uenum) -> int64_t
    {
        ...
        for (auto [Name, Value] : uenum->ForEachName())
        {
            auto enum_name = sanitize_enumeration_name(to_system_string(Name.ToString()));
                                                             ...
                                                             int32_t* out_num_params) -> SystemStringType
    {
        ...
        for (FProperty* property : function->ForEachProperty())
        {
            ...
            if ((property_flags & CPF_Parm) != 0 && (property_flags & CPF_ReturnParm) == 0)
            {
                ...
                auto property_name = to_system_string(property->GetName());
    ...
    auto UEHeaderGenerator::generate_default_property_value(FProperty* property, GeneratedSourceFile& header_data, const SystemStringType& ContextName)
            -> SystemStringType
    {
        const auto field_class_name = (property->GetClass().GetName());
        ...
        if (field_class_name == STR("MapProperty"))
        {
            ...
            const auto key_type = generate_property_type_declaration(key_property, context);
            const auto value_type = generate_property_type_declaration(value_property, context);
        ...
        throw std::runtime_error(
                RC::fmt("[generate_default_property_value] Unsupported property class '{}', full name: '{}'", field_class_name, property->GetFullName()));
    }
    ...
    auto UEHeaderGenerator::get_module_name_for_package(UObject* package) -> SystemStringType
    {
        ...
        auto package_name = to_system_string(package->GetName());
    ...
    auto UEHeaderGenerator::generate_object_description_file(UObject* object) -> bool
    {
        ...
        {
            throw std::runtime_error(RC::fmt("Provided object {} is not of a supported type: {}", object->GetName(), object->GetClassPrivate()->GetName()));
        }
    ...
    auto UEHeaderGenerator::generate_cross_module_include(UObject* object, const SystemStringType& module_name, const SystemStringType& fallback_name)
            -> SystemStringType
    {
