...
#pragma warning(default: 4251)
    ...
    public:
        ...
        auto new_string(const StringType& value) -> void;
        auto new_null() -> void;
        ...
        auto for_each(Callable callable) const -> void
        {
            for (decltype(m_members)::size_type i = 0; i < m_members.size(); ++i)
            {
                ...
                {
                    if (callable(i, *element.get()) == LoopAction::Break) { break; }
                }
                else
                {
                    if (callable(*element.get()) == LoopAction::Break) { break; }
                }
            }
        }
    ...
    public:
        ...
        auto get_type() const -> Type override { return Type::Array; }
    };
}
