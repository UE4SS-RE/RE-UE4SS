...
namespace RC::JSON
{
    ...
    private:
        ...
        auto static get_internal(ValueType stored_type, StoredValueType& value) -> NumberType&
        {
            ...
            {
                if (stored_type == Type::UInt32) { return *std::bit_cast<NumberType*>(&value.as_integer); }
            }
            else if constexpr (std::is_same_v<NumberType, uint64_t>)
            {
                if (stored_type == Type::UInt64) { return *std::bit_cast<NumberType*>(&value.as_integer); }
            }
            else if constexpr (std::is_same_v<NumberType, int32_t>)
            {
                if (stored_type == Type::Int32) { return *std::bit_cast<NumberType*>(&value.as_integer); }
            }
            else if constexpr (std::is_same_v<NumberType, int64_t>)
            {
                if (stored_type == Type::Int64) { return *std::bit_cast<NumberType*>(&value.as_integer); }
            }
            else if constexpr (std::is_same_v<NumberType, float>)
            {
                if (stored_type == Type::Float) { return value.as_float; }
            }
            else if constexpr (std::is_same_v<NumberType, double>)
            {
                if (stored_type == Type::Double) { return value.as_double; }
            }
        ...
        auto static is_internal(ValueType stored_type) -> bool
        {
            ...
            if constexpr (std::is_same_v<NumberType, uint32_t>)
            {
                if (stored_type == Type::UInt32) { return true; }
            }
            else if constexpr (std::is_same_v<NumberType, uint64_t>)
            {
                if (stored_type == Type::UInt64) { return true; }
            }
            else if constexpr (std::is_same_v<NumberType, int32_t>)
            {
                if (stored_type == Type::Int32) { return true; }
            }
            else if constexpr (std::is_same_v<NumberType, int64_t>)
            {
                if (stored_type == Type::Int64) { return true; }
            }
            else if constexpr (std::is_same_v<NumberType, float>)
            {
                if (stored_type == Type::Float) { return true; }
            }
            else if constexpr (std::is_same_v<NumberType, double>)
            {
                if (stored_type == Type::Double) { return true; }
            }
            else
            {
                return false;
    ...
    public:
        ...
        auto get_type() const -> JSON::Type override { return JSON::Type::Number; }
    };
}
