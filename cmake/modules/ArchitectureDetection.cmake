...
# Convenience function for SSE2 detection
function(detect_sse2 target)
    set(options DISABLE_ON_CROSS_COMPILE)
    set(oneValueArgs DEFINE_IF_DISABLED DEFINE_IF_ENABLED)
    set(multiValueArgs)
    cmake_parse_arguments(PARSE_ARGV 1 SSE2 "${options}" "${oneValueArgs}" "${multiValueArgs}")
    
    set(cross_compile_arg "")
    if(SSE2_DISABLE_ON_CROSS_COMPILE)
        set(cross_compile_arg "DISABLE_ON_CROSS_COMPILE")
    endif()
    
    detect_and_configure_feature(${target}
        FEATURE sse2
        GNU_FLAG -msse2
        ARCHITECTURES "x86_64|AMD64|i[3-6]86|x86"
        DEFINE_IF_DISABLED "${SSE2_DEFINE_IF_DISABLED}"
        DEFINE_IF_ENABLED "${SSE2_DEFINE_IF_ENABLED}"
        ALWAYS_AVAILABLE_ON "x86_64|AMD64"
        ${cross_compile_arg}
    )
endfunction()

# Convenience function for AVX2 detection
function(detect_avx2 target)
    set(options)
    set(oneValueArgs DEFINE_IF_DISABLED DEFINE_IF_ENABLED)
    set(multiValueArgs)
    cmake_parse_arguments(PARSE_ARGV 1 AVX2 "${options}" "${oneValueArgs}" "${multiValueArgs}")
    ...
    detect_and_configure_feature(${target}
        ...
        GNU_FLAG -mavx2
        ARCHITECTURES "x86_64|AMD64|i[3-6]86|x86"
        DEFINE_IF_DISABLED "${AVX2_DEFINE_IF_DISABLED}"
        DEFINE_IF_ENABLED "${AVX2_DEFINE_IF_ENABLED}"
    )
endfunction()

# Convenience function for NEON detection (ARM)
function(detect_neon target)
    set(options)
    set(oneValueArgs DEFINE_IF_DISABLED DEFINE_IF_ENABLED)
    set(multiValueArgs)
    cmake_parse_arguments(PARSE_ARGV 1 NEON "${options}" "${oneValueArgs}" "${multiValueArgs}")
    
    detect_and_configure_feature(${target}
        FEATURE neon
        GNU_FLAG -mfpu=neon
        ARCHITECTURES "arm|armv7|armv7-a|aarch64|arm64"
        DEFINE_IF_DISABLED "${NEON_DEFINE_IF_DISABLED}"
        DEFINE_IF_ENABLED "${NEON_DEFINE_IF_ENABLED}"
        ALWAYS_AVAILABLE_ON "aarch64|arm64"  # ARMv8 always has NEON
    )
endfunction()