...
#1: SetSharedVariable(string VariableName, any Value))"};
            ...
            if (auto it = RC::LuaMod::m_shared_lua_variables.find(variable_name); it != RC::LuaMod::m_shared_lua_variables.end()) { currently_stored_value = &it->second; }

            auto type = lua_type(lua.get_lua_state(), 1);
            if (type == LUA_TNIL) { RC::LuaMod::m_shared_lua_variables[variable_name] = RC::LuaMod::SharedLuaVariable{type, nullptr, false}; }
            ...
            else if (type == LUA_TLIGHTUSERDATA)
            {
                ...
                RC::LuaMod::m_shared_lua_variables[variable_name] = RC::LuaMod::SharedLuaVariable{type, new RC::LuaMod::SharedLuaVariable::UserdataContainer{lua_touserdata(lua.get_lua_state(), 1)}, false};
            }
            ...
            else if (type == LUA_TTABLE) { lua.throw_error("Tried to set shared variable but the variable is unsupported type: 'table'."); }
            else if (type == LUA_TFUNCTION) { lua.throw_error("Tried to set shared variable but the variable is unsupported type: 'function'."); }
            else if (type == LUA_TUSERDATA)
            {
                ...
                if (lua_object_name == "UObject" || lua_object_name == "World" || lua_object_name == "Actor" || lua_object_name == "UClass" || lua_object_name == "UEnum" || lua_object_name == "UScriptStruct" ||
                    lua_object_name == "UStruct")
                {
                    RC::LuaMod::m_shared_lua_variables[variable_name] = RC::LuaMod::SharedLuaVariable{type, new RC::LuaMod::SharedLuaVariable::UserdataContainer{userdata.get_remote_cpp_object()}, false};
                }
                else { lua.throw_error("Tried to set shared variable but the variable is unsupported type: 'userdata' other than UObject derivative."); }
            }
            else if (type == LUA_TTHREAD) { lua.throw_error("Tried to set shared variable but the variable is unsupported type: 'thread'."); }
...
#1: GetSharedVariable(string VariableName))"};
            ...
            if (auto it = RC::LuaMod::m_shared_lua_variables.find(variable_name); it != RC::LuaMod::m_shared_lua_variables.end())
            {
                ...
                if (it->second.lua_type == LUA_TNIL) { lua.set_nil(); }
                else if (it->second.lua_type == LUA_TBOOLEAN) { lua.set_bool(*static_cast<bool*>(value)); }
                else if (it->second.lua_type == LUA_TLIGHTUSERDATA) { lua_pushlightuserdata(lua.get_lua_state(), static_cast<RC::LuaMod::SharedLuaVariable::UserdataContainer*>(value)->userdata); }
                else if (it->second.lua_type == LUA_TNUMBER)
                {
                    if (it->second.is_integer) { lua.set_integer(*static_cast<int64_t*>(value)); }
                    else { lua.set_number(*static_cast<double*>(value)); }
                }
                else if (it->second.lua_type == LUA_TSTRING) { lua.set_string(static_cast<std::string*>(value)->c_str()); }
                else if (it->second.lua_type == LUA_TTABLE) { lua.throw_error("Tried to get shared variable but the variable is unsupported type: 'table'."); }
                else if (it->second.lua_type == LUA_TFUNCTION) { lua.throw_error("Tried to get shared variable but the variable is unsupported type: 'function'."); }
                else if (it->second.lua_type == LUA_TUSERDATA) { auto_construct_object(lua, static_cast<Unreal::UObject*>(static_cast<RC::LuaMod::SharedLuaVariable::UserdataContainer*>(value)->userdata)); }
                else if (it->second.lua_type == LUA_TTHREAD) { lua.throw_error("Tried to get shared variable but the variable is unsupported type: 'thread'."); }
            }
            else { lua.set_nil(); }
