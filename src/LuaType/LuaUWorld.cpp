...
namespace RC::LuaType
{
    ...
    auto UWorld::setup_member_functions(const LuaMadeSimple::Lua::Table& table, std::string_view metatable_name) -> void
    {
...
#1: SpawnActor(UClass Class, table Location, table Rotation))"};
            ...
            else if (lua.is_table())
            {
                lua.for_each_in_table([&](const LuaMadeSimple::LuaTableReference& table) {
                    if (table.key.is_string() && table.key.get_string() == "X" && table.value.is_number()) { location.SetX(table.value.get_number()); }

                    if (table.key.is_string() && table.key.get_string() == "Y" && table.value.is_number()) { location.SetY(table.value.get_number()); }

                    if (table.key.is_string() && table.key.get_string() == "Z" && table.value.is_number()) { location.SetZ(table.value.get_number()); }
                    return false;
                });
            }
            ...
            {
                lua.for_each_in_table([&](const LuaMadeSimple::LuaTableReference& table) {
                    if (table.key.is_string() && table.key.get_string() == "Yaw" && table.value.is_number()) { rotation.Yaw = table.value.get_number(); }

                    if (table.key.is_string() && table.key.get_string() == "Pitch" && table.value.is_number()) { rotation.Pitch = table.value.get_number(); }

                    if (table.key.is_string() && table.key.get_string() == "Roll" && table.value.is_number()) { rotation.Roll = table.value.get_number(); }
                    return false;
                });
            }
            else { lua.throw_error(error_overload_not_found); }
