...
#2: FName(integer ComparisonIndex))"};
    ...
    auto FName::setup_member_functions(LuaMadeSimple::Lua::Table& table, std::string_view metatable_name) -> void
    {
        ...
        table.add_pair("Equals", [](const LuaMadeSimple::Lua& lua) -> int {
            if (!lua.is_userdata(1) || !lua.is_userdata(2)) { lua.throw_error("FName.Equals called but there was not two userdata to compare (use ':' to call, not '.')"); }
