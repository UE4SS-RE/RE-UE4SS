...
namespace RC::UEGenerator
{
    ...
    auto UEHeaderGenerator::generate_enum_definition(UEnum* uenum, GeneratedSourceFile& header_data) -> void
    {
        ...
        for (auto [Name, Value] : uenum->ForEachName()) 
        {
            ...
            if (pre_append_result_line.ends_with(STR("_MAX")))
            {
                ...
                if ((pre_append_result_line == expected_full_constant_name ||
                    pre_append_result_line == sanitize_enumeration_name(expected_full_constant_name)) &&
    ...
    auto UEHeaderGenerator::generate_object_implementation(UClass* uclass, GeneratedSourceFile& implementation_file) -> void
    {
        ...
        if (encountered_replicated_properties)
        {
            ...
            implementation_file.append_line(std::format(STR("void {}::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const {{"), class_native_name));
            implementation_file.begin_indent_level();
    ...
    auto UEHeaderGenerator::generate_function(UClass* uclass, UFunction* function, GeneratedSourceFile& header_data, bool is_generating_interface, const CaseInsensitiveSet& blacklisted_property_names, bool generate_as_override) -> void
    {
        ...
        header_data.append_line(std::format(STR("{}{} {}({}){};"), function_modifier_string, return_property_string, function->GetName(), function_argument_list, function_extra_postfix_string));
        header_data.append_line(STR(""));
    }
    ...
    auto UEHeaderGenerator::generate_simple_assignment_expression(FProperty* property, const std::wstring& value, GeneratedSourceFile& implementation_file, const std::wstring& property_scope, const std::wstring& operator_type) -> void
    {
    ...
    auto UEHeaderGenerator::generate_advanced_assignment_expression(FProperty* property, const std::wstring& value, GeneratedSourceFile& implementation_file, const std::wstring& property_scope, const std::wstring& property_type, const std::wstring& operator_type) -> void
    {
        const std::wstring field_class_name = property->GetName();
        ...
        else
        {
            for (int32_t i = 0; i < property->GetArrayDim(); i++)
            {
                implementation_file.append_line(std::format(STR("*p_{}->ContainerPtrToValuePtr<{}>(this){}[{}]{}{};"), field_class_name, property_scope, field_class_name, i, operator_type, value));
            }
        }
    }
            ...
            if (sub_object_value->HasAnyFlags(EObjectFlags::RF_DefaultSubObject))
            {
                ...
                if (auto it = m_class_subobjects.find(object_name); it != m_class_subobjects.end())
                {
                    ...
                    if (prior_private)
                    {
                        ...
                        implementation_file.append_line(std::format(STR("FProperty* p_{}_Prior = GetClass()->FindPropertyByName(\"{}\");"), initializer, initializer));
                            ...
                            if (check_property->IsA<FObjectProperty>())
                            {
                                ...
                                if (check_sub_object_value)
                                {
                                    ...
                                    if (check_object_name == attach_parent_object_name)
                                    {
                                        ...
                                        {
                                            implementation_file.append_line(std::format(STR("FProperty* p_{}_Parent = GetClass()->FindPropertyByName(\"{}\");"), check_property->GetName(), check_property->GetName()));
                                            attach_string = std::format(STR("SetupAttachment(p_{}_Parent->ContainerPtrToValuePtr<{}>(this))"), check_property->GetName(), check_sub_object_value->GetClassPrivate()->GetName());
                                        }
                                        parent_found = true;
                                        break;
                                    }
    ...
    auto UEHeaderGenerator::generate_function_implementation(UClass* uclass, UFunction* function, GeneratedSourceFile& implementation_file, bool is_generating_interface, const CaseInsensitiveSet& blacklisted_property_names) -> void
    {
        ...
        if (!function_implementation_name.empty())
        {
            ...
            implementation_file.append_line(std::format(STR("{} {}({}){} {{"), return_value_type, function_implementation_name,
