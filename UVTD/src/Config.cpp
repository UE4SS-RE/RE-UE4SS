...
namespace RC::UVTD
{
    ...
    inline std::string_view TypeFilterCategoryToString(TypeFilterCategory category)
    {
        switch (category)
        {
        case TypeFilterCategory::CompleteExclusion:
            return "complete_exclusion";
        case TypeFilterCategory::ExcludeFromGetters:
            return "exclude_from_getters";
        case TypeFilterCategory::ExcludeFromSolBindings:
    ...
    struct MethodOverloadMappingJson
    {
...
namespace glz
{
    ...
    struct meta<RC::UVTD::ObjectItemJson>
    {
        static constexpr auto value = glz::object(
                "name",
                &RC::UVTD::ObjectItemJson::name,
                "valid_for_vtable",
                &RC::UVTD::ObjectItemJson::valid_for_vtable,
                "valid_for_member_vars",
                &RC::UVTD::ObjectItemJson::valid_for_member_vars
                );
    };
    ...
    struct meta<RC::UVTD::MemberRenameInfoJson>
    {
        static constexpr auto value = glz::object(
                "mapped_name",
                &RC::UVTD::MemberRenameInfoJson::mapped_name,
                "generate_alias_setter",
                &RC::UVTD::MemberRenameInfoJson::generate_alias_setter,
                "description",
                &RC::UVTD::MemberRenameInfoJson::description
                );
    };
    ...
    struct meta<RC::UVTD::ClassInheritanceInfoJson>
    {
        static constexpr auto value = glz::object(
                "parent_class",
                &RC::UVTD::ClassInheritanceInfoJson::parent_class,
                "version_major",
                &RC::UVTD::ClassInheritanceInfoJson::version_major,
                "version_minor",
                &RC::UVTD::ClassInheritanceInfoJson::version_minor,
                "inherit_members",
                &RC::UVTD::ClassInheritanceInfoJson::inherit_members
                );
    };

    // Glaze metadata for method overload mapping
    template <>
    struct glz::meta<RC::UVTD::MethodOverloadMappingJson>
    {
        static constexpr auto value = glz::object(
                "unique_id",
                &RC::UVTD::MethodOverloadMappingJson::unique_id,
                "return_type",
                &RC::UVTD::MethodOverloadMappingJson::return_type,
                "param_types",
                &RC::UVTD::MethodOverloadMappingJson::param_types,
                "is_const",
                &RC::UVTD::MethodOverloadMappingJson::is_const,
                "mapped_name",
                &RC::UVTD::MethodOverloadMappingJson::mapped_name
                );
    };
}
...
namespace RC::UVTD
{
    ...
    bool UVTDConfig::Initialize(const std::filesystem::path& config_dir)
    {
        ...
        try
        {
            ...
            if (std::filesystem::exists(variants_path))
            {
                ...
                if (result.has_value())
                {
                    ...
                    Output::send(STR("Loaded case preserving variants: {} non-preserving, {} preserving\n"),
                                 non_case_preserving_variants.size(),
                                 case_preserving_variants.size());
                }
                else
                {
            ...
            // Load method overload mappings
            std::filesystem::path method_overload_path = config_dir / "method_overload_mappings.json";
            if (std::filesystem::exists(method_overload_path))
            {
                std::ifstream file(method_overload_path);
                std::string json_str((std::istreambuf_iterator<char>(file)), std::istreambuf_iterator<char>());

                using MethodOverloadMapJson = std::unordered_map<std::string,
                                                                 std::unordered_map<std::string,
                                                                                    std::vector<MethodOverloadMappingJson>>>;
                auto result = glz::read_json<MethodOverloadMapJson>(json_str);

                if (result.has_value())
                {
                    method_overload_mappings.clear();

                    for (const auto& [class_name, method_map] : result.value())
                    {
                        auto wide_class_name = to_wstring(class_name);
                        std::unordered_map<File::StringType, std::vector<MethodOverloadMapping>> wide_method_map;

                        for (const auto& [method_name, overloads] : method_map)
                        {
                            auto wide_method_name = to_wstring(method_name);
                            std::vector<MethodOverloadMapping> wide_overloads;

                            for (const auto& overload : overloads)
                            {
                                MethodOverloadMapping wide_overload{
                                        to_wstring(overload.unique_id),
                                        to_wstring(overload.return_type),
                                        {}, // Will convert param types
                                        overload.is_const,
                                        to_wstring(overload.mapped_name)
                                };

                                // Convert parameter types
                                for (const auto& param : overload.param_types)
                                {
                                    wide_overload.param_types.push_back(to_wstring(param));
                                }

                                wide_overloads.push_back(wide_overload);
                            }

                            wide_method_map[wide_method_name] = wide_overloads;
                        }

                        method_overload_mappings[wide_class_name] = wide_method_map;
                    }

                    Output::send(STR("Loaded method overload mappings for {} classes\n"),
                                 method_overload_mappings.size());
                }
                else
                {
                    Output::send(STR("Failed to parse method_overload_mappings.json\n"));
                }
            }
            else
            {
                Output::send(STR("method_overload_mappings.json not found\n"));
            }
