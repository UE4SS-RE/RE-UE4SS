...
namespace RC::UVTD
{
    auto UnrealVirtualGenerator::generate_files() -> void
    {
        ...
        std::map<File::StringType, std::vector<const PlatformMemberLayout*>> platform_layouts_by_class;
        for (const auto& layout : ConfigUtil::GetPlatformMemberLayouts())
        {
            if (layout.version != base_version)
            {
                continue;
            }
            for (const auto& [class_name, _] : layout.classes)
            {
                platform_layouts_by_class[class_name].emplace_back(&layout);
            }
        }

        for (auto& [class_name, platform_layouts] : platform_layouts_by_class)
        {
            std::ranges::sort(platform_layouts, [](const PlatformMemberLayout* left, const PlatformMemberLayout* right) {
                return std::tie(left->platform, left->ifdef_macro) < std::tie(right->platform, right->ifdef_macro);
            });

            std::unordered_set<File::StringType> emitted_macros;
            for (const auto* layout : platform_layouts)
            {
                if (!emitted_macros.emplace(layout->ifdef_macro).second)
                {
                    throw std::runtime_error{std::format(
                        "Duplicate platform member layout macro '{}' for version '{}' and class '{}'",
                        to_utf8_string(layout->ifdef_macro),
                        to_utf8_string(base_version),
        ...
        const auto generate_member_layout_include = [&virtual_src_dumper, &platform_layouts_by_class](
            [[maybe_unused]] const File::StringType& lookup_version,
            const File::StringType& fallback_filename_prefix,
            const File::StringType& class_name) {
            static const std::vector<const PlatformMemberLayout*> no_platform_layouts;
            const auto platform_layouts_it = platform_layouts_by_class.find(class_name);
            const auto& platform_layouts = platform_layouts_it == platform_layouts_by_class.end()
                                                   ? no_platform_layouts
                                                   : platform_layouts_it->second;

            for (size_t index = 0; index < platform_layouts.size(); ++index)
            {
                const auto& layout = *platform_layouts[index];
                virtual_src_dumper.send(index == 0 ? STR("#if {}\n") : STR("#elif {}\n"), layout.ifdef_macro);
                virtual_src_dumper.send(
                    STR("#include <FunctionBodies/Platform/{}/{}_MemberVariableLayout_DefaultSetter_{}.cpp>\n"),
                    layout.platform,
                    layout.version,
                    class_name);
            }

            if (!platform_layouts.empty())
            {
        ...
        for (const auto& object_item : ConfigUtil::GetObjectItems())
        {
            ...
            if (!class_entry.variables.empty())
            {
                generate_member_layout_include(base_version, base_version, class_name_clean_final);
            }
        }
