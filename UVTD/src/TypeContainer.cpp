...
namespace RC::UVTD
{
    ...
    auto TypeContainer::get_or_create_class_entry(const UEStringType& symbol_name, const UEStringType& symbol_name_clean, const SymbolNameInfo& name_info) -> Class&
    {
        auto& class_entry = [&]() -> auto& {
            ...
            {
                return class_entries.emplace(symbol_name_clean, Class{.class_name = UEStringType{symbol_name}, .class_name_clean = symbol_name_clean}).first->second;
            }
        }();
