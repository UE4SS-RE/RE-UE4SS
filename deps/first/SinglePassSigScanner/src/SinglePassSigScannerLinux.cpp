...
namespace RC
{
    ...
    auto SinglePassScanner::scanner_work_thread(uint8_t* start_address, uint8_t* end_address, DLData& info, std::vector<SignatureContainer>& signature_containers)
            -> void
    {
    ...
    {
        //DLData info = SigScannerStaticData::m_modules_info[ScanTarget::MainExe];
        ...
        if (!SigScannerStaticData::m_is_modular)
        {
            ...
            if (merged_module_info.size >= m_multithreading_module_size_threshold)
            {
                ...
                uint32_t range = merged_module_info.size / m_num_threads;
            ...
            else
            {
                ...
                uint8_t* module_end_address = static_cast<uint8_t*>(module_start_address + merged_module_info.size);
        ...
        else
        {
            ...
            {
                uint8_t* module_start_address = static_cast<uint8_t*>(SigScannerStaticData::m_modules_info[scan_target].base_address);
                uint8_t* module_end_address = static_cast<uint8_t*>(module_start_address + SigScannerStaticData::m_modules_info[scan_target].size);
