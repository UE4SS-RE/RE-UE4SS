...
struct LogEntry {
    ...
    {
        if (!is_heap_allocated && message_length > 0) {
            std::memcpy(inline_buffer.data(), other.inline_buffer.data(),
                        (message_length + 1) * sizeof(CharType));
        }
...
struct ThreadQueue {
    ...
    explicit ThreadQueue(size_t queue_size)
        : queue(queue_size) {}
};
...
public:
    ...
    auto get_total_enqueued() const -> size_t { return m_total_enqueued.load(); }
    auto get_total_flushed() const -> size_t { return m_total_flushed.load(); }
    auto get_total_dropped() const -> size_t { return m_total_dropped.load(); }
