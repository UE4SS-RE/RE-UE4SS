...
namespace RC::Output
{
    ...
    auto AsyncLogger::start(uint64_t flush_every_n_loops, size_t batch_size, size_t batch_wake_threshold, std::chrono::milliseconds flush_interval, size_t default_queue_size)
            -> void
    {
    ...
    auto AsyncLogger::stop() -> void
    {
        ...
        // Worker is stopped, we are now the sole consumer - safe to drain directly
        drain_all_queues();
    }
    ...
    auto AsyncLogger::try_enqueue(LogEntry&& entry) -> bool
    {
        ...
        {
            // Use lvalue reference to avoid moving on failed enqueue.
            // The queue will copy on success; we only clear entry after confirmed success.
            if (tls_state.queue->queue.try_enqueue(entry))
            {
    ...
    auto AsyncLogger::flush() -> void
    {
        ...
        {
            // Worker not running, we can drain directly
            drain_all_queues();
            return;
        }

        // Request worker to flush and wait for completion
        m_flush_completed.store(false, std::memory_order_relaxed);
    ...
    auto AsyncLogger::process_exit_requests() -> void
    {
        std::thread::id tid;
        while (m_exit_requests.try_dequeue(tid))
        {
            std::vector<LogEntry> remaining_entries;
            remaining_entries.reserve(m_default_queue_size);

            {
                std::unique_lock queues_lock{m_queues_mutex};

                auto it = std::find_if(m_queues.begin(), m_queues.end(), [tid](const auto& tq) {
                    return tq->thread_id == tid;
                });
    ...
    auto AsyncLogger::worker_loop(std::stop_token stoken) -> void
    {
        ...
        {
            // Process any pending thread exit requests first
            process_exit_requests();

            // Check for flush request
            bool flush_requested = m_flush_requested.exchange(false, std::memory_order_acq_rel);
            ...
            bool should_flush = flush_requested || nudged ||
            ...
            // Drain all queues if flush was explicitly requested
            if (flush_requested)
            {
                drain_all_queues();
            ...
            buffer.clear();
            {
                std::shared_lock queues_lock{m_queues_mutex};
                ...
                if (num_queues > 0)
                {
                    for (size_t i = 0; i < num_queues && buffer.size() < m_batch_size; ++i)
                    {
                        ...
                        {
                            size_t dequeued =
                                    tq->queue.dequeue_bulk(m_dequeue_buffer.data(), std::min<size_t>(m_dequeue_buffer.size(), m_batch_size - buffer.size()));
        ...
        // Process any remaining exit requests before shutdown
        process_exit_requests();
    }
    ...
    {
        // This should only be called by the worker thread OR after worker has stopped
